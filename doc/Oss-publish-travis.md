...
2. Sending the key to keyserver: Use the command, `gpg --send-key <KEY_FROM_THE_LAST_COMMAND> hkp://pool.sks-keyservers.net`. The command will finish execution instantaneously, if not make sure you have your firewall disabled. 
Instead of using the <KEY_FROM_THE_LAST_COMMAND>, you can use the RSA KEY ID, this can be fetched using the command on any encrypted files generated (e.g.,` gpg --verify target/scala-2.10/biglibrary_2.10-0.0.3.pom.asc`).
